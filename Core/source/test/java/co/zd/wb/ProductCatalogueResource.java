			...
			.openResource(resourceId, "MySqlDatabase", "Product Catalogue Database")
				.openStates()
					.openState(stateIdDatabaseCreated, "Database created")
						.openAssertions()
							.openAssertion("MySqlDatabaseExists", assertionIdDatabaseExists, "Database exists")
							.closeAssertion()
						.closeAssertions()
					.closeState()
					.openState(stateIdCoreSchemaLoadedId, "Core Schema Loaded")
						.openAssertions()
							.openAssertion("MySqlTableExists", assertionIdProductTypeTableExists, "Table exists")
								.openElement("tableName").text("ProductType").closeElement("tableName")
							.closeAssertion()
							.openAssertion("MySqlTableExists", assertionIdProductTableExists, "Product table exists")
					...
					.openState(stateIdInitialReferenceDataLoaded, "Reference Data Loaded")
						.openAssertions()
							.openAssertion("RowExists", assertionIdProductTypeHwRowExists, "ProductType HW exists")
								.openElement("sql").openCdata()
									.text("SELECT * FROM ProductType WHERE ProductTypeCode = 'HW';")
								.closeCdata().closeElement("sql")
							.closeAssertion()
							.openAssertion("RowExists", assertionIdProductTypeSwRowExists, "ProductType SW exists")
								.openElement("sql").openCdata()
