...
public class AnsiSqlTableDoesNotExistDomAssertionBuilder extends BaseDomAssertionBuilder
{
		...
		int seqNum) throws MessagesException
	{
		...
		if (!schemaName.hasValue()) { V.elementMissing(messages, assertionId, "schemaName", AnsiSqlTableDoesNotExistAssertion.class); }
		if (!tableName.hasValue()) { V.elementMissing(messages, assertionId, "tableName", AnsiSqlTableDoesNotExistAssertion.class); }
